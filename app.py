...
def google_oauth_callback():
    ...
    try:
        if role == "student":
            ...
            # Log in the student
            app.logger.info(f"Logging in student: {student.email} (ID: {student.id})")
            login_user(student, remember=True)
            app.logger.info(f"Student logged in. is_authenticated: {current_user.is_authenticated}")
...
def test_auth():
    ...
    return f"""
    <h1>Auth Status Test</h1>
    <p>Is Authenticated: {current_user.is_authenticated}</p>
    <p>User: {current_user if current_user.is_authenticated else 'Anonymous'}</p>
    <p>Email: {current_user.email if current_user.is_authenticated else 'N/A'}</p>
    <p>Name: {current_user.name if current_user.is_authenticated else 'N/A'}</p>
    <p>Role: {current_user.role if current_user.is_authenticated else 'N/A'}</p>
    <hr>
    <a href="/">Home</a> | <a href="/explore">Explore</a> | <a href="/logout">Logout</a>
    """
